...
<details>
    ...
    Самый первый процесс при запуске системы получает пид 1(PID 1), так называемый инит(init) или его современный эквивалент, чаще всего systemd<br />
...
<details>
  <summary>Что такое ProcFS?</summary><br> 
     ProcFS (сокр. от Process File System) — это виртуальная (или псевдо-) файловая система в Unix-подобных системах, которая монтируется в каталог /proc. <br>
     Ее главное назначение — служить динамическим интерфейсом между ядром (kernel) операционной системы и программами пользовательского пространства.<br>
     Ключевые особенности ProcFS: <br>
     Виртуальная природа: Файлы и каталоги в /proc не хранятся на реальном жестком диске. Они создаются "на лету" ядром Linux и существуют только во время работы системы.<br>
     Интерфейс к ядру: Чтение или запись в эти виртуальные файлы позволяет получать информацию о текущем состоянии системы, оборудовании, запущенных процессах и изменять некоторые параметры ядра во время выполнения.<br>
     Динамичность: Содержимое /proc постоянно обновляется, отражая актуальное состояние системы.  <br> <br>
</details>
...
<details>
  <summary>Сколько адресов по маске /16</summary><br> 
     <b> Общее количество бит в IPv4-адресе составляет 32 бита. <br>
       Маска /16 означает, что первые 16 бит используются для адреса сети: Биты для хостов: 32 -16<br>
       Количество адресов рассчитывается по формуле \(2^{N}\), где \(N\) — количество бит, доступных для хостов: \(2^{16}=65536\text{\ адресов}\) </b><br>
</details>
...
<details>
    <summary>Что такое probes для пода? Какой порядок запуска?</summary><br> 
     <b> Startup Probe (Проверка запуска) - проверяет завершился ли процесс запуска приложения внутри контейнера. Эта проба предназначена для приложений, которым требуется много времени для инициализации (например, загрузка больших кэшей, миграция базы данных при первом запуске).  </b> <br>
     <b> Liveness Probe (Проверка активности/живучести) проверяет работает ли контейнер правильно после запуска. Эта проба используется на протяжении всего времени работы пода. Цель: Обнаружить зависшие или неработоспособные контейнеры (например, с deadlocks) и перезапустить их.</b> <br>
     <b> Readiness Probe (Проверка готовности) проверяет готов ли контейнер принимать сетевой трафик. Эта проба также используется на протяжении всего времени работы пода, но ее фокус — это готовность обрабатывать запросы от сервисов. Цель: Исключить неготовые поды из балансировки нагрузки сервиса (Service) до тех пор, пока они не будут полностью готовы обрабатывать запросы. </b> <br>
</details>
